...
function menu_select(i_url) {
  ...
  //
  if (
    (i_url == g_subpage_currentUrl && g_subpage_goingToUrl == null) ||
    i_url == g_subpage_goingToUrl
  )
    return;
  ...
  slideToSubpage(i_url, function () {
    ...
    if (
      document
        .getElementById("subpageContent")
        .getElementsByTagName("iframe")[0].contentWindow.startCode
    )
      document
        .getElementById("subpageContent")
        .getElementsByTagName("iframe")[0]
        .contentWindow.startCode();
  });
}
...
function menu_positionOfUrl(i_url) {
  ...
  var subpages = $("#menu a")
    .map(function (i_index, i_element) {
...
function menu_adjacentUrl(i_fromUrl, i_direction) {
  ...
  if (fromPosition == -1) return null;

  fromPosition += i_direction;
  if (fromPosition < 0 || fromPosition >= subpages.length) return null;
...
function window_onKeyDown(i_event) {
  ...
  if (i_event.key == "PageDown" || i_event.key == "ArrowRight") {
    var adjacentUrl = menu_adjacentUrl(
      g_subpage_goingToUrl || g_subpage_currentUrl,
      1
    );
    if (adjacentUrl) menu_select(adjacentUrl);
...
function slideToSubpage(i_url, i_onDone) {
  ...
  var currentSubpagePathname = document
    .getElementById("subpageContent")
    .getElementsByTagName("iframe")[0].src;
  currentSubpagePathname = currentSubpagePathname.slice(
    currentSubpagePathname.lastIndexOf("/") + 1
  );
  ...
  var slide = 0;
  if (newSubpagePosition > currentSubpagePosition) slide = 1;
  else if (newSubpagePosition < currentSubpagePosition) slide = -1;
...
function goToSubpage(i_url, i_slide, i_onDone) {
  ...
  if (i_slide != 0) newContent.style.visibility = "hidden";
  ...
  contentIframe.addEventListener("load", function (i_event) {
    pageIsLoaded = true;
    if (pageIsLoaded && pageIsInView) onFullyDone();
  ...
  if (i_slide == 0) {
    ...
    dan.movement.animate(slideDurationInSeconds, function (i_progress) {
      ...
      if (i_progress == 0) newContent.style.visibility = "visible";
