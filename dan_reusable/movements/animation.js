// This namespace
// #require "movement.js"
if (!window.dan) window.dan = {};
if (!window.dan.movement) window.dan.movement = {};
...
//      Cancel animation; don't call the callback again.
{
  ...
  function step() {
    ...
    if (!callbackWantsToCancel && progress < 1) requestAnimationFrame(step);
  }

  step();
};

dan.movement.animate_delayedStart = function (
  i_startInSeconds,
  i_durationInSeconds,
  i_onStep
) // Fixed length animation
...
//    Don't use a callback.
{
  dan.movement.animate(i_durationInSeconds, function (i_progress) {
    if (i_easingFunction) i_progress = i_easingFunction(i_progress);

    //
    i_element.style.opacity = i_progress;

    // If finished and a callback was passed then call it
    if (i_progress == 1 && i_onDone) i_onDone();
  });
};
