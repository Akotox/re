...
function addOneshotEventListener(
  i_target,
  i_eventName,
  i_handler,
  i_useCapture
...
function logVideoEvents(i_videoElement) {
  i_videoElement.addEventListener(
    "abort",
    function (
      i_event
  ...
  i_videoElement.addEventListener(
    "ended",
    function (i_event) // Sent when playback completes.
    {
  ...
  i_videoElement.addEventListener(
    "loadeddata",
    function (i_event) // The first frame of the media has finished loading.
    {
  ...
  i_videoElement.addEventListener(
    "loadstart",
    function (i_event) // Sent when loading of the media begins.
    {
  ...
  i_videoElement.addEventListener(
    "pause",
    function (i_event) // Sent when playback is paused.
    {
  ...
  i_videoElement.addEventListener(
    "ratechange",
    function (i_event) // Sent when the playback speed changes.
    {
  ...
  i_videoElement.addEventListener(
    "seeked",
    function (i_event) // Sent when a seek operation completes.
    {
  ...
  i_videoElement.addEventListener(
    "seeking",
    function (i_event) // Sent when a seek operation begins.
    {
...
function videoReadyEventName() {
  ...
  if (isiPhone() || isiPad()) {
    ...
    if (iosVersion && iosVersion[0] <= 7) return "suspend";
...
function resizeVideoToContainer(
  i_videoElement,
  i_defaultHorizontalAspect,
  i_defaultVerticalAspect
) {
  ...
  var containerTop = parseInt(
    document.defaultView.getComputedStyle($container[0]).top.replace("px", "")
  );
...
function addPlayButton() {
  ...
  if (jQuery(".videoPlayButton").length > 0) return;
